const tx = 'a';
const weather_api = `https://api.open-meteo.com/v1/forecast?latitude=${tx}&longitude=${tx}&current=temperature_2m,is_day,apparent_temperature&timezone=auto&forecast_days=1`;
...
export function parseCountryInformation(countryInformation, weatherInfo) {
  ...
  //to do: calculate with locale current hour in compare with sunrise/sunset
  const isDay = weatherInfo?.current?.isDay || true;
...
export async function getWeatherInformationForCountry(latitude, longitude) {
  try {
    const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=sunrise,sunset&current=temperature_2m,is_day,apparent_temperature&timezone=auto`;
    const response = await fetch(url);
