...
export class UI {
  ...
  bindListeners() {
    this.stateManager.subscribe('countries', this.pageManager);
    this.stateManager.subscribe('favoritesList', this.favoritesListElement);
    this.stateManager.subscribe('historyList', this.historyListElement);
  }
  ...
  async searchHandler(isTextFromInput, textFromHistory) {
    ...
    try {
      ...
      this.stateManager.dispatch('update-Countries', fullData, (data) =>
        this.pageManager.updateData(data),
      );
      this.stateManager.dispatch('', searchValue, (list, text) => {
  ...
  favoriteIconHandler(iconID) {
    ...
    favoriteIcon.addEventListener('click', (event) => {
      ...
      const action = isCountryInFavoriteList(countryName)
      ...
      this.stateManager.dispatch(action, payload, (list, state) => {
        changeFavoriteState(state.country, state.event);
        showListInPage(this.favoritesListElement, list);
      });
    });
  }
